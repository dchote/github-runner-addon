...
- AWS access keys / secrets / session tokens are rejected in `extra_env` (they would persist in backups). The manager never bind-mounts host `~/.aws`. It does **not** disable IMDS — see [0008](0008-no-host-aws-credentials.md).
