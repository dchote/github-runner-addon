...
- **Expected config**: records in `/data/runners.json` (name, URL, labels, container/volume names, runtime limits). Registration tokens and PATs are not stored in JSON; registration tokens are passed to the container env only until registration succeeds.

## Core User Flows

1. **Create runner** — enter name, GitHub project URL, and either a registration token or rely on a configured PAT; optional labels and runtime limits.
...
3. **Edit** — update labels, image, resources, env, and docker-sock override; optionally apply by recreating the container.
4. **Logs** — view and follow the runner container’s stdout/stderr.
5. **Delete** — remove local container and volume; with PAT, also deregister from GitHub when possible.
