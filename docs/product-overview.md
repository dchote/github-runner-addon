...
- **Persistent cache**: optional named volume or host bind (default `/cache`) for incremental CI; share across runners by reusing the same volume name or host path. For sibling Docker / Buildx that use the mount path on the host, prefer a bind with `host_path` equal to the mount path (commonly both `/cache`).
