...
5. Monitor status in the table; use **Edit**, **Logs**, **Recreate**, and lifecycle actions as needed. When a container is **running**, the Status column shows **idle** / **busy** / **unknown** (job activity from local hooks, no PAT). Details show current job fields while busy. While **busy**, the UI disables Edit, Delete, and Recreate (start/stop/restart/logs stay available). After a Docker reset, use **Recreate missing** when the Missing chip is greater than zero (token or PAT if data volumes are gone).

Registration tokens are passed only to a short-lived configure container (token + no-op command so the image configures without listening), then the long-running listener starts without them. They are not stored in `runners.json`. The PAT is never written into runner containers.
...
Registration credentials live in a Docker named volume per runner (`*-data`). Job workdir is a **host directory** same-path bind (default `/srv/gha-work/<name>`) so sibling `docker run -v $GITHUB_WORKSPACE` works with the Docker socket — not a Docker volume `_data` path. Optional **persistent cache** is a host same-path bind (UI default) or named volume. Bind caches mount `host_path` at the same path in the runner and set `RUNNER_CACHE` to that path for workflows / Buildx `type=local`. Named volumes are not visible to sibling host binds (soft `warnings[]`). Large volumes are **not** part of HA `/data` backups. See [0006](../docs/features/0006-same-path-build-cache.md).

**Recreate / Save & apply:** remounts the host workdir and sets `RUNNER_WORKDIR`. If the agent `.runner` `workFolder` does not match, the addon backs up `.runner`, clears it, and reconfigures (PAT or registration token required); a failed start restores the backup. Store updates for Save & apply happen only after a successful recreate. After start, a mismatched `workFolder` fails the request (fail closed). Deleting a runner does **not** delete the host workdir tree. After a Docker engine reset (containers and named volumes gone), Recreate or **Recreate missing** with a token or PAT re-registers from `runners.json`; leftover host `status.json` does not block recreate when the container is missing. Start/restart/logs on a missing container return **400** — recreate it. Custom `:local` images must exist on the host (they are not pulled). Images that ignore Docker `CMD` may start listening during the configure phase — use myoung34 or an entrypoint that execs CMD after `config.sh`.
